...
import { runAgentLoop } from "@/lib/cs-agent-loop";
import {
...
/**
 * One CSM agent run, independent of how it was triggered.
 *
 * ⚠ THIS LIVES OUTSIDE THE ROUTE SO THE NIGHTLY JOB CAN CALL IT. The route is
 * `requireCsLayer`-gated and cron arrives with no session, so leaving the logic
 * in the handler would have meant a second copy — and two copies of a decision
 * agent drift, which is exactly the problem the shared loop was extracted to
 * avoid.
 *
 * ⚠ IT NEVER SENDS. No send tool is defined, so the loop cannot reach one, and
 * `lib/gmail-send.ts` sends only as the signed-in user — an unattended run has
 * no credential to send with even if someone added one.
 *
 * ⚠ EVERY RULE IN 08's "code enforces" COLUMN IS CHECKED IN `checkProposal`,
 * not in the prompt. A proposal that fails is handed back to the model with the
 * reason and costs a tool call, so a model that keeps proposing invalid
 * outreach runs out of budget rather than looping.
 */

export interface CsmRunInput {
  customerNsId: string;
  trigger: "manual" | "nightly";
  runBy: string;
  /** Reuse a row the queue already created, instead of inserting a new one. */
  existingRunId?: string;
}

export type CsmRunResult =
  | { ok: false; status: number; error: string; needsProfile?: boolean; runId?: string }
  | {
      ok: true; runId: string;
      outcome: "proposed" | "skipped" | "blocked";
      category?: string; reason?: string;
      draft?: unknown; suppression?: unknown; lint?: string[];
      humanFlag: { note: string; urgency: string } | null;
      toolCalls: number; partial: boolean; stopReason: string;
...
export async function runCsmAgent(input: CsmRunInput): Promise<CsmRunResult> {
  const { customerNsId, trigger, runBy, existingRunId } = input;
  const supabase = getSupabaseAdmin();

  const apiKey = process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return { ok: false, status: 503, error: "ANTHROPIC_API_KEY is not set." };
  ...
  if (!res) return { ok: false, status: 404, error: "No active customer with that id." };
  ...
  if (!snapshot.hasProfile) {
    return {
      ok: false, status: 422, needsProfile: true,
  ...
  // The nightly worker has already created and claimed its row; a manual run
  // creates one here. Either way there is exactly one row per run, and it is
  // marked `running` before the first model call rather than after.
  let run: { id: string };
  if (existingRunId) {
    await supabase.from("cs_agent_runs").update({
      status: "running", model: CSM_MODEL, prompt_version: CSM_PROMPT_VERSION,
      started_at: new Date().toISOString(), customer_name: res.customerName,
    }).eq("id", existingRunId);
    run = { id: existingRunId };
  } else {
    const { data: created, error: runErr } = await supabase.from("cs_agent_runs").insert({
    ...
    if (runErr) return { ok: false, status: 503, error: runErr.message };
    run = created;
  }
  ...
  try {
    ...
    if (!output) {
      ...
      return { ok: false, status: 502, runId: run.id, error: "The agent finished without deciding." };
    }
    ...
    if (output.kind === "skip") {
      ...
      return {
        ok: true, runId: run.id, outcome: "skipped",
