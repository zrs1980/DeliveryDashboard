...
- **The fetch walks BACKWARDS from `to` in windows — it does not page the whole range with `skip`.** The old version paged from `skip: 0`, capped at 500, and only sorted newest-first *after* truncating, on the unverified assumption that Fireflies returns newest-first. It does not, so on a busy range the cap discarded the **newest** meetings and the grid silently showed only old ones. Windowing makes Fireflies' internal ordering irrelevant: each window is date-bounded and the newest window is taken first, so the fetch can stop as soon as it has enough.
  - Returns **the most recent `limit`** (default 100, `DEFAULT_MEETING_LIMIT`). `truncated` now only ever means *older* meetings weren't fetched — never newer ones.
  - Window size adapts: starts at 14 days, doubles (to 120) whenever a window returns less than half a page, snaps back when one fills. Widening only on an *empty* window left a sparse account spending a request per fortnight; with the half-page test, every simulated density — busy, quiet, sparse, empty — covers a full year or reaches the limit in ≤6 requests. Windows are `[start, end]` with the next ending at `start - 1`, so there are no gaps or overlaps.
  - `MAX_REQUESTS` (16) is the hard ceiling per load. **Rate limits are per *day* on lower plans**: Free 50/day, Pro 500/day, Business/Enterprise 60/min. Don't add polling or auto-refresh — the tab loads once and relies on the Refresh button.
  - The default `from` is now **today − 365 days**, computed per request. It was hardcoded `"2026-07-01"`, which quietly widened as time passed. It is a *search bound*, not a display window — the walk stops once it has `limit` meetings.
