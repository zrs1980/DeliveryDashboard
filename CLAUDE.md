...
- **Fixed-fee projects count ALL actual time, not just billable.** On a fixed-fee engagement the client isn't billed per hour, so consultants log time as non-billable *by design* — billable-only would leave the project reading as fully unspent no matter how much work went in. Project **413** (Salt and Stone Phase 2) has 132.8h of actual time, none of it billable; counting it gives **117.3h remaining, matching NetSuite's own field exactly**. These rows carry an **FF** chip and a tooltip showing the arithmetic.
  - The list is `FIXED_FEE_PROJECT_IDS` in `lib/constants.ts`, **not** `jobbillingtype`. That field exists and is populated account-wide (FBI 99 · FBM 37 · TM 206) but every *active* project reads `TM`, including 413. Switching to it would also catch project **268**, which carries FBI and has 910.3h against a **0h** budget — swinging it from 0.0h to −910.3h. Set `jobbillingtype` correctly in NetSuite, check what it does to 268, then flip the helper and empty the list.
  - **A non-fixed-fee project whose time is all non-billable still shows its full budget as remaining.** That follows from measuring billable spend and is not a bug — but it means the column answers "how much billable budget is left", not "how much work is left". The greyed **B** chip in the Class column is the tell; if such a project is actually fixed fee, add its ID to the list.
...
- **Two renderings, and the consultant table needs `NoteLine`, not `NoteChip`.** In the by-project table the project row is unconstrained, so the inline `NoteChip` is fine. In the consultant (Section 1) table the project cell is `nowrap` + `textOverflow: ellipsis` at `maxWidth: 440`, and the type chip + B/U/P chips + "Placeholder Project for Capacity Planning" already fill it — an inline chip there is clipped away entirely and reads as "the note isn't showing". `NoteLine` puts it on its own line under the title, where it also wraps.
...
- `custentity_project_remaining_hours` is manually maintained by PMs and can lag behind logged time. Show a data-integrity warning when `timebill total > budget_hours - remaining_hours` by more than 20h — but **that comparison MUST use actual time only (`tb.timetype = 'A'`)**.
  - **The "Salt & Stone has severe drift" note that used to live here was wrong.** It compared NetSuite's consumed figure against an unfiltered `timebill` sum, which includes `'B'` (allocated) rows — a forecast, not work done. Verified August 2026 across all 11 active projects: with the filter applied, logged actual hours equal NetSuite's consumed figure **exactly** on every one, and the warning fires on none. Unfiltered it fired on 7 of 11, e.g. Oxide 784h "logged" against 313h actually worked.
  - `timetype` values in this account: `'A'` actual (131k h), `'B'` allocated (5k h), `'P'` planned (1.9k h). Only `'A'` is worked time.
...
- **Fireflies returns transcripts NEWEST-FIRST, and `skip` pages that order correctly.** Verified against the live API (August 2026): `transcripts(limit: 10)` came back strictly descending by date, and a full 100-meeting fetch produced **0 out-of-order pairs**. So the fetch pages from `skip: 0` and stops once it has `limit` — two requests for the default 100.
  - Returns **the most recent `limit`** (default 100, `DEFAULT_MEETING_LIMIT`); `truncated` means older meetings weren't loaded, never newer ones.
  - The sort-then-slice at the end is a guard, not decoration: if Fireflies ever changed this ordering we'd still return the newest of what was fetched. But we could then miss newer meetings entirely — **so if this tab ever shows stale meetings again, re-check the ordering first.**
  - A previous version walked backwards in adaptive date windows, on the theory that the order was oldest-first and the old 500-row cap was discarding the newest. That theory was wrong; the ordering had simply never been checked in either direction. Don't reintroduce windowing without evidence — it cost 4–6 requests to do what 2 do.
  - The default `from` is **today − 365 days**, computed per request (it was hardcoded `"2026-07-01"`, which quietly widened as time passed). It's a *search bound*, not a display window.
  - `MAX_PAGES` (10) is the ceiling per load. **Rate limits are per *day* on lower plans**: Free 50/day, Pro 500/day, Business/Enterprise 60/min. Don't add polling or auto-refresh — the tab loads once and relies on the Refresh button.
- **"No recent meetings" is usually Fireflies, not the dashboard.** On 5 Aug 2026 the newest transcript in the whole workspace was 31 July: the notetaker had recorded nothing for five days across 22 users. Before touching the fetch code, query the API directly and check what the newest transcript actually is.
