...
export function SRDashboardView() {
  ...
  const { consultants, teamThisMonth, teamLastMonth, teamYTD, teamQuota, attainmentPct, monthHistory, employeeIds } = data;
            ...
            >
              ...
              <div style={{ marginBottom: 8 }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: C.textSub, marginBottom: 4 }}>
                  ...
                  <span style={{ fontFamily: C.mono, fontWeight: 700, color: ragColor(c.rag) }}>{c.thisMonth} / {c.quota}</span>
                </div>
              ...
              {expanded === c.id && (
                <div style={{ marginTop: 12, borderTop: `1px solid ${ragBd(c.rag)}`, paddingTop: 10 }}>
                  ...
                  <div style={{ display: "flex", gap: 6 }}>
                    {c.history.map(h => (
                      <div key={h.key} style={{ flex: 1, textAlign: "center" }}>
                        <div style={{ fontSize: 14, fontWeight: 800, fontFamily: C.mono, color: h.count >= c.quota ? C.green : h.count >= 2 ? C.yellow : C.red }}>{h.count}</div>
                ...
                return (
                  <tr key={row.key} style={{ background: isCurrentMonth ? C.blueBg : i % 2 === 0 ? "#fff" : C.alt, borderBottom: `1px solid ${C.border}` }}>
                    ...
                    {consultants.map(c => {
                      ...
                      return (
                        <td key={c.id} style={{ padding: "9px 14px", textAlign: "center", fontFamily: C.mono, fontWeight: 700, color: count >= 3 ? C.green : count >= 2 ? C.yellow : count > 0 ? C.red : C.textSub }}>
                          {count || "—"}
                        </td>
                      );
                    })}
